...
const DetailAfterScan = (props: any) => {
  ...
  useEffect(() => {
    ...
    var month =
      new Date(itemShow?.up_date_statuses[0]?.inspected_at).getMonth() + 1; //Current Month
    var year =
      new Date(itemShow?.up_date_statuses[0]?.inspected_at).getFullYear() + 543; //Current Year
    var hours = new Date(
      itemShow?.up_date_statuses[0]?.inspected_at,
    ).getHours(); //Current Hours
    var min = new Date(
      itemShow?.up_date_statuses[0]?.inspected_at,
  ...
  const onClickSave = async () => {
    if (itemShow?.departmentDId == getProfile?.departmentDId || conditionSave) {
      ...
      dataform.append('locationLId', !valueLocations ? '' : valueLocations);
      dataform.append('status', statusNew);
      dataform.append('note', detailProblem + ' แก้ไขโดย Mobile');
      ...
      try {
        const res = await axios(
          await configAxios(
            'post',
            `${api}`,
            imageProblem.image1 != null ? dataform : data,
          ),
        );
        setResData(res);
        setShowSuccess(true);
  ...
  const [afterSelectStatus, setAfterSelectStatus] = useState<any>({
    showImageProblem: false,
  });
          ...
          {itemShow.name_image_item ? (
            <View
              ...
              <Image
                ...
                source={{
                  uri: `${baseURL}${PATH_IMAGE_ITEM}${itemShow.name_image_item}`,
                }}
              />
            </View>
          ) : (
          ...
          <View
            ...
            <View style={{flexDirection: 'row', marginTop: 20}}>
              ...
              <View
                ...
                <Text style={globleStyles.fontstatus}>
            ...
            <View style={{marginHorizontal: 15}}>
              <View style={{marginVertical: 5}}>
                <Text style={globleStyles.fonts}>
                  ผู้ดูแล : {itemShow?.profile?.firstname}{' '}
                  {itemShow?.profile?.lastname}
                </Text>
              </View>
              <View style={{marginVertical: 5}}>
                <Text style={globleStyles.fonts}>
                  รหัสครุภัณฑ์ : {itemShow?.code}
                </Text>
              </View>
              <View style={{marginVertical: 5}}>
                <Text style={globleStyles.fonts}>
                  หมวดหมู่ครุภัณฑ์ : {itemShow?.typeitem?.name}{' '}
                </Text>
              </View>
            </View>
            ...
            <View style={{flex: 1}}>
              <View style={styles.item_BoxDetail}>
                <View style={styles.rowDetail}>
                  <Text style={globleStyles.fonts}>
                    รายละเอียดครุภัณฑ์ : {itemShow?.description}{' '}
                  </Text>
                </View>
                <View style={styles.rowDetail}>
                  <Text style={globleStyles.fonts}>
                    วันที่รับเข้า :{' '}
                    {itemShow?.createdAt == null ? '-' : createdAtdate}
                  </Text>
                </View>

                <View style={styles.rowDetail}>
                  <Text style={globleStyles.fonts}>
                    สถานที่ : {location_nameTH}{' '}
                  </Text>
                </View>
                <View style={styles.rowDetail}>
                  <Text style={globleStyles.fonts}>
                    ตรวจสอบครั้งล่าสุด :{' '}
                    {itemShow?.up_date_statuses[0] == null ? '-' : currentDate}
                  </Text>
                </View>
                <View style={styles.rowDetail}>
                  <Text style={globleStyles.fonts}>
                    หมายเหตุ :{' '}
                    {itemShow?.up_date_statuses[0] == null ||
                    ...
                    <>
                      ...
                      <View
                        ...
                        <TouchableOpacity
                          ...
                          <Text style={styles.fontBTStatus}>ตรวจสอบสถานะ</Text>
                        </TouchableOpacity>
                      </View>
                    </>
                  )}
