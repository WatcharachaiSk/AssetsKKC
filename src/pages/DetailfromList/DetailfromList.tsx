...
const DetailfromList = (props: any) => {
    ...
    if (isPage == 'Category')
      try {
        const res = await axios(
          await configAxios('get', `${API.getItemById}${item.item_id}`),
        );
  ...
  useEffect(() => {
    ...
    if (isPage != 'Category') {
      for (let i = 0; i < item?.img_items?.length; i++) {
        arrImage.push({
          img: `${baseURL}${PATH_IMAGE_ITEM}${item?.img_items[i].name_image_item}`,
        });
      }
      setImageItems(arrImage);
    } else {
      ...
      if (getItemByID) {
        for (let i = 0; i < getItemByID?.img_items?.length; i++) {
          arrImage.push({
            img: `${baseURL}${PATH_IMAGE_ITEM}${getItemByID?.img_items[i].name_image_item}`,
          });
        }
        setImageItems(arrImage);
      }
  ...
  return (
    <ScrollView contentContainerStyle={{flexGrow: 1}}>
      ...
      <View style={{flex: 1}}>
        {getItemByID != undefined && <BottomSheet itemShow={getItemByID} />}
        {isPage !== 'Category' && <BottomSheet itemShow={item} />}
      </View>
    </ScrollView>
  );
};
