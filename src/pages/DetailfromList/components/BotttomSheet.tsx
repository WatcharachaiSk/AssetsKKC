  ...
  return (
    <View style={{flex: 1, justifyContent: 'flex-end', alignItems: 'center'}}>
      ...
      <RBSheet
        ...
        height={height > 600 ? height - 200 : height - 150}
        animationType="slide"
        customStyles={{
          container: {
        ...
        <ScrollView>
            ...
            <View style={{flex: 0, width: widthOfWindow * 0.3}}>
              <View
                style={[
                  styles.status,
                  {
                    backgroundColor: chackStatusItemColor(
                      itemShow?.status_item,
                    ),
                  },
                ]}></View>
              <Text style={globleStyles.fontstatus}>
                {chackStatusItem(itemShow?.status_item)}
              </Text>
            </View>
          </View>
          {/*  */}
          ...
          <View style={{marginHorizontal: 15}}>
            ...
            <View style={{marginVertical: 5}}>
              <Text style={globleStyles.fonts}>
                หมวดหมู่ครุภัณฑ์ :
                {itemShow?.typeitem == null ? '-' : itemShow?.typeitem.name}{' '}
              </Text>
            </View>
            <View style={{marginVertical: 5}}>
              <Text style={globleStyles.fonts}>
                วันที่รับเข้า :{' '}
                {itemShow?.typeitem == null ? '-' : createdAtdate}{' '}
              </Text>
            </View>
            <View style={{marginVertical: 5}}>
              <Text style={globleStyles.fonts}>
                สถานที่ : ห้อง{itemShow?.location.nameTH} ชั้น{' '}
                {itemShow?.location?.floor} อาคาร {itemShow?.building?.nameTH}{' '}
              </Text>
            </View>
            <View style={{marginVertical: 5}}>
              <Text style={globleStyles.fonts}>
                ตรวจสอบครั้งล่าสุด :{' '}
                {itemShow?.up_date_statuses[0]?.inspected_at != null
                  ? currentDate
                  : '-'}{' '}
              </Text>
            </View>
            <View style={{marginVertical: 5}}>
              <Text style={globleStyles.fonts}>
                หมายเหตุ : {itemShow?.up_date_statuses[0]?.note}{' '}
              </Text>
            </View>
            ...
            <View
              ...
              <Image
                style={{
                  width:
                    height > 600 ? widthOfWindow * 0.5 : widthOfWindow * 0.5,
                  height:
                    height > 600 ? heightOfWindow * 0.25 : heightOfWindow * 0.3,
                  flex: 0,
                }}
                source={{
                  uri: `${baseURL}${PATH_IMAGE_ITEM_DAMAGED}${itemShow?.name_image_damaged}`,
                }}
              />
            </View>
          </View>
